...
<div class="container mx-auto px-4 py-8 mt-20">
        ...
        <table class="min-w-full bg-white rounded-xl shadow-lg">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-right text-blue-600 font-semibold">${{ number_format($item['price'], 2) }}</td>
                    <td class="px-6 py-4 text-center">
                        <div class="inline-flex items-center border rounded-lg">
                            <button class="decrease-qty px-2 py-1 text-lg text-gray-500 hover:text-blue-600 focus:outline-none" data-service-id="{{ $id }}">-</button>
                            <input type="number" min="1" class="cart-qty-input w-12 text-center font-semibold" data-service-id="{{ $id }}" value="{{ $item['qty'] }}">
                            <button class="increase-qty px-2 py-1 text-lg text-gray-500 hover:text-blue-600 focus:outline-none" data-service-id="{{ $id }}">+</button>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-right font-bold text-green-600 cart-item-subtotal">${{ number_format($item['price'] * $item['qty'], 2) }}</td>
                    <td class="px-6 py-4 text-center">
                        <button class="remove-from-cart text-red-500 hover:text-red-700" data-service-id="{{ $id }}" title="Remove">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mt-8 gap-4">
        <a href="{{ route('service.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg font-semibold transition">Continue Shopping</a>
        <a href="{{ route('payment.index') }}" class="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-10 py-4 rounded-lg font-bold text-lg shadow-lg hover:shadow-xl transition">Proceed to Checkout</a>
    </div>
    @endif
</div>
@endsection
        ...
        $('.remove-from-cart').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.cart-qty-badge').text(response.total_qty);
                    $('.add-service-to-cart[data-service-id="' + response.service_id + '"] .service-qty-badge').text(response.service_qty);
