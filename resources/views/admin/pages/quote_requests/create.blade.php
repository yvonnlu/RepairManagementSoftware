...
<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ isset($quoteRequest) ? 'Edit' : 'Create' }} Quote Request</h1>
            <p class="text-gray-600">{{ isset($quoteRequest) ? 'Update quote request details' : 'Add a new quote request' }}</p>
        </div>
    ...
    <div class="max-w-4xl">
        <form action="{{ isset($quoteRequest) ? route('admin.quote-requests.update', $quoteRequest) : route('admin.quote-requests.store') }}" 
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="status" id="status" 
                                ...
                                required>
                            <option value="pending" {{ old('status', $quoteRequest->status ?? 'pending') === 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="quoted" {{ old('status', $quoteRequest->status ?? '') === 'quoted' ? 'selected' : '' }}>Quoted</option>
                            <option value="completed" {{ old('status', $quoteRequest->status ?? '') === 'completed' ? 'selected' : '' }}>Completed</option>
                            <option value="rejected" {{ old('status', $quoteRequest->status ?? '') === 'rejected' ? 'selected' : '' }}>Rejected</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        <label for="is_existing_customer" class="block text-sm font-medium text-gray-700 mb-1">Customer Type</label>
                        <select name="is_existing_customer" id="is_existing_customer" 
                            ...
                            <option value="0" {{ old('is_existing_customer', $quoteRequest->is_existing_customer ?? false) == '0' ? 'selected' : '' }}>New Customer</option>
                            <option value="1" {{ old('is_existing_customer', $quoteRequest->is_existing_customer ?? false) == '1' ? 'selected' : '' }}>Existing Customer</option>
                        </select>
