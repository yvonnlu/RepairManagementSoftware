                ...
                <div class="flex items-center space-x-2">
                    <form method="GET" action="" class="flex items-center space-x-2">
                        ...
                        <select name="status" onchange="this.form.submit()"
                            ...
                            <option value="new_order" {{ request('status') === 'new_order' ? 'selected' : '' }}>New
                                Order</option>
                            <option value="diagnosing" {{ request('status') === 'diagnosing' ? 'selected' : '' }}>
                                Diagnosing</option>
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if ($order->deleted_at)
                                            ...
                                            <form method="POST" action="{{ route('admin.order.restore', $order->id) }}"
                                                class="inline" id="restore-order-form-{{ $order->id }}">
                                                @csrf
                                                <button type="button"
                                                    onclick="confirmRestore('Order #{{ $order->id }}', function() { document.getElementById('restore-order-form-{{ $order->id }}').submit(); })"
                                        ...
                                        @else
                                            ...
                                            <form method="POST" action="{{ route('admin.order.destroy', $order->id) }}"
                                                class="inline" id="delete-order-form-{{ $order->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                                    onclick="confirmDelete('Order #{{ $order->id }}', function() { document.getElementById('delete-order-form-{{ $order->id }}').submit(); })"
